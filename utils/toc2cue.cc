...
int convertBin(Toc *toc, char *oldBin, char *newBin, int byteSwap)
{
    ...
    for (trun = titr.first(start, end), trackNr = 1; trun != NULL;
...
{
    message(1, "toc2cue version %s - (C) Andreas Mueller <andreas@daneb.de>", VERSION);
    message(1, "");
}
...
{
    message(
        0, "Usage: %s [-v #] [-s] [-C output-bin-file] { -V | -h | input-toc-file output-cue-file}",
        PRGNAME);
}
...
static int parseCommandLine(int argc, char **argv, char **tocFile, char **cueFile,
                            char **convertedBinFile, int *byteSwap)
{
    int c;
    int printVersion = 0;
    int printHelpInfo = 0;
...
int main(int argc, char **argv)
{
    ...
    switch (
        parseCommandLine(argc, argv, &tocFile, &cueFile, &convertedBinFile, &byteSwapBinAudio)) {
    case 0: // error
        printUsage();
        return 1;
        break;
    ...
    for (trun = titr.first(start, end), trackNr = 1; trun != NULL;
        ...
        int stcount;
        TrackData::Type sttype1 = TrackData::DATAFILE, sttype2 = TrackData::DATAFILE;
        ...
        for (strun = stitr.first(), stcount = 0; strun != NULL; strun = stitr.next(), stcount++) {
    ...
    message(1, "Converted toc-file '%s' to cue file '%s'.", tocFile, cueFile);
    message(1, "");
