...
Track::Track(TrackData::Mode t, TrackData::SubChannelMode st) : length_(0), start_(0), end_(0)
{
    type_ = t;
    subChannelType_ = st;
    audioCutMode_ = -1;
...
Track::Track(const Track &obj)
    : length_(obj.length_), start_(obj.start_), end_(obj.end_), cdtext_(obj.cdtext_)
{
    int i;
    SubTrack *run;
        ...
        do {
            ...
            if ((padLen = slength % blen) != 0) {
                ...
                if (audioCutMode_)
                    ...
                    pad =
                        new SubTrack(SubTrack::PAD, 0, TrackData(dataMode, subChannelMode, padLen));
...
void Track::print(std::ostream &out, PrintParams &params) const
{
    ...
    if (type() == TrackData::AUDIO) {
        ...
        if (isrcValid()) {
            out << "ISRC \"" << isrcCountry(0) << isrcCountry(1) << isrcOwner(0) << isrcOwner(1)
                << isrcOwner(2) << (char)(isrcYear(0) + '0') << (char)(isrcYear(1) + '0')
...
SubTrack *Track::findSubTrack(unsigned long sample) const
{
    ...
    if (audioCutMode()) {
        ...
        if (sample >= length_.lba() * TrackData::dataBlockSize(type(), subChannelType()))
            return NULL;
    }

    for (run = subTracks_; run != NULL && run->next_ != NULL; run = run->next_) {
...
int Track::moveIndex(int index, long lba)
{
    ...
    rangeMax = (index == nofIndices_ - 1 ? length_.lba() - start_.lba() : index_[index + 1].lba());
...
TrackDataList *Track::removeFromStart(unsigned long sample)
{
    ...
    for (strun = subTracks_; strun != NULL && strun->next_ != NULL; strun = strun->next_) {
...
TrackDataList *Track::removeTrackData(unsigned long start, unsigned long end)
{
    ...
    if (startSt == endSt) {
        if (start == startSt->start() && end == startSt->start() + startSt->length() - 1) {
            ...
            insertSubTrackAfter(startSt->pred_, new SubTrack(startSt->type(), *part1));
            plen = part1->length();
            delete part1;
...
void Track::mergeSubTracks()
{
    ...
    while (run != NULL && run->next_ != NULL) {
        if (run->type() == run->next_->type() && (mergedData = run->merge(run->next_)) != NULL) {
...
void Track::encodeZeroData(int encMode, TrackData::Mode mode, TrackData::SubChannelMode smode,
...
long TrackReader::readData(int encodingMode, int subChanEncodingMode, long lba, char *buf, long len)
{
    long err = 0;
    long b;
    long offset;
    ...
    for (b = 0; b < len; b++) {
        if ((offset = readBlock(encodingMode, subChanEncodingMode, lba, (Sample *)buf)) == 0) {
            err = 1;
            break;
        }
        buf += offset;
...
//
int TrackReader::readBlock(int encodingMode, int subChanEncodingMode, long lba, Sample *buf)
{
    ...
    if (encodingMode == 0) {
        ...
        case TrackData::MODE1_RAW: {
            ...
            if (int2bcd(m.min()) != encBuf[12] || int2bcd(m.sec()) != encBuf[13] ||
...
int TrackReader::seekSample(unsigned long sample)
{
    ...
    if (track_->audioCutMode() == 0) {
        ...
        unsigned long blen = TrackData::dataBlockSize(track_->type(), track_->subChannelType());
...
long TrackReader::readSamples(Sample *buf, long len)
{
    ...
    if (track_->type() == TrackData::AUDIO) {
        if (track_->audioCutMode() == 1) {
            ...
            while (len > 0) {
                long burst = SAMPLES_PER_BLOCK - (readPosSample_ % SAMPLES_PER_BLOCK);
                int fullBurst = 0;
