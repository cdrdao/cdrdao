...
FormatSupport::setup_wav_output(const std::string& wav_file)
{
    ...
    if (!ao_dev_) {
	log_message(-2, "Unable to create output file \"%s\": %s",
		    wav_file.c_str(), strerror(errno));
...
FormatSupport::Status
FormatSupport::write_wav_output(char* samples, u32 num_bytes)
{
    assert(ao_dev_);
...
FormatSupport::Status
FormatSupport::close_wav_output()
{
    if (ao_dev_) {
	ao_close(ao_dev_);
	ao_dev_ = nullptr;
