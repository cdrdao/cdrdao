...
int main(int argc, char *argv[])
{
	...
	for (; 1; sector++) {
		...
		memcpy(inbuf_copy, inbuf, 2352);


#if 0
		/* check CRC value */
		if (memcmp(inbuf_copy+2348, "\0\0\0", 4) != 0
		    || build_edc(inbuf_copy,16,16+8+2324-1) != 0U)
			continue;
#endif

		/* clear fields,  which shall be build */
		memset(inbuf_copy, 0, 16);
		memset(inbuf_copy+16+8+2324, 0, 4);

		/* build fields */
		do_encode_L2(inbuf_copy, MODE_2_FORM_2, address++);
