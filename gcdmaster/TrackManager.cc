...
{
    if (iterator_ == NULL || (iterator_ = iterator_->next) == NULL)
        return NULL;
    else
        return iterator_->ent;
}
...
const TrackManager::Entry *TrackManager::pick(gint x, gint *stopXMin, gint *stopXMax)
{
    EntryList *run;
    Entry *pred;
...
void TrackManager::update(const Toc *toc, unsigned long start, unsigned long end, gint width)
{
    ...
    if (toc == NULL || start == end || (t = itr.find(start, tstart, tend, &trackNr)) == NULL)
        return;

    double samp2pix = double(width - 1) / double(end - start);
#define lba2pixel(lba) gint(double(((lba) * SAMPLES_PER_BLOCK) - start) * samp2pix + 0.5)
    ...
    while ((t = itr.next(tstart, tend)) != NULL) {
        ...
        if (t->start().lba() != 0) {
            ...
            append(new Entry(t, trackNr, 0, lba2pixel(tstart.lba() - t->start().lba())));
        }
