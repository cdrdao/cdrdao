...
static const unsigned char scsi_command_size[8] = {6, 10, 10, 12, 12, 12, 10, 10};
...
void sg_print_sense(const char *leadin, const unsigned char *sense_buffer)
{
    ...
    if (sense_class == 7) { /* extended sense data */
        s = sense_buffer[7] + 8;
        if (s > SG_ERR_MAX_SENSE_LEN)
            s = SG_ERR_MAX_SENSE_LEN;

        if (!valid)
            log_message(0, "[valid=0] ");
        log_message(0, "Info fld=%d, ",
                    (int)((sense_buffer[3] << 24) | (sense_buffer[4] << 16) |
        ...
        for (i = 0; additional[i].text; i++)
            if (additional[i].code1 == sense_buffer[12] && additional[i].code2 == sense_buffer[13])
                log_message(0, "Additional sense indicates: %s", additional[i].text);
...
static const char *hostbyte_table[] = {"DID_OK",
                                       "DID_NO_CONNECT",
                                       "DID_BUS_BUSY",
                                       "DID_TIME_OUT",
                                       "DID_BAD_TARGET",
...
static const char *driversuggest_table[] = {
    "SUGGEST_OK", "SUGGEST_RETRY", "SUGGEST_ABORT", "SUGGEST_REMAP", "SUGGEST_DIE",
...
void sg_print_driver_status(int driver_status)
{
    ...
    int i;
    int dr = driver_status & DRIVER_MASK;
    int su = (driver_status & SUGGEST_MASK) >> 4;
    ...
    log_message(0, "(%s,%s) ", dr < driver_max ? driverbyte_table[dr] : "invalid",
                su < suggest_max ? driversuggest_table[su] : "invalid");
}

int sg_chk_n_print(const char *leadin, int target_status, int host_status, int driver_status,
                   const unsigned char *sense_buffer)
{
    int done_leadin = 0;
    int done_sense = 0;

    if ((0 == target_status) && (0 == host_status) && (0 == driver_status))
    ...
    if (0 != target_status) {
        ...
        log_message(0, "");
        if (sense_buffer &&
            ((target_status == CHECK_CONDITION) || (target_status == COMMAND_TERMINATED))) {
...
int sg_err_category(int target_status, int host_status, int driver_status,
                    const unsigned char *sense_buffer)
{
    if ((0 == target_status) && (0 == host_status) && (0 == driver_status))
        return SG_ERR_CAT_CLEAN;
    if ((CHECK_CONDITION == target_status) || (COMMAND_TERMINATED == target_status) ||
        (DRIVER_SENSE & driver_status)) {
        if (sense_buffer) {
    ...
    if (0 != host_status) {
        if ((DID_NO_CONNECT == host_status) || (DID_BUS_BUSY == host_status) ||
            (DID_TIME_OUT == host_status))
            return SG_ERR_CAT_TIMEOUT;
    }
    if (0 != driver_status) {
        if (DRIVER_TIMEOUT == driver_status)
            return SG_ERR_CAT_TIMEOUT;
    }
