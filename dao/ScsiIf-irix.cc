...
#define PREFIX_SC "/dev/scsi/sc"
    ...
    if (len == 0) {
        impl_->name_ = NULL;
    } else if (sscanf(name, "%i,%i,%i%n", &bus, &targ, &lun, &count) == 3 && count == len) {
    ...
    } else if (strncmp(name, "/", 1) == 0 || strncmp(name, "./", 2) == 0 ||
               strncmp(name, "../", 3) == 0) {
...
#define ERROR(msg)                                                                                 \
    ...
    /* for printError: */
    impl_->dsreq_.ds_cmdbuf = NULL;
    impl_->dsreq_.ds_cmdlen = 0;
    ...
    if (impl_->dsreq_.ds_ret == 0 || impl_->dsreq_.ds_ret == DSRT_OK ||
...
void ScsiIf::printError()
{
    if (impl_->dsreq_.ds_cmdbuf != NULL) {
        ...
        case DSRQ_READ:
            p += snprintf(p, s + sizeof(s) - p, " RD");
            break;
        case DSRQ_WRITE:
            p += snprintf(p, s + sizeof(s) - p, " WR");
            break;
        case DSRQ_READ | DSRQ_WRITE:
            p += snprintf(p, s + sizeof(s) - p, " RW");
            break;
        }
    ...
    if (impl_->error_ != NULL) {
        ...
        case DSRT_DEVSCSI:
            message(-2, "devscsi failure");
            break;
        case DSRT_MULT:
            message(-2, "request rejected");
            break;
        case DSRT_CANCEL:
            message(-2, "request cancelled");
            break;
        case DSRT_REVCODE:
            message(-2, "obsolete");
            break;
        case DSRT_AGAIN:
            ...
            break;
        case DSRT_HOST:
            message(-2, "host failure");
            break;
        case DSRT_NOSEL:
            message(-2, "no select");
            break;
        case DSRT_SHORT:
            message(-2, "short transfer");
            break;
        case DSRT_OK:
            message(-2, "complete transfer");
            break;
        case DSRT_SENSE: /* message(-2,"sense"); */
            decodeSense(impl_->sensebuf_, impl_->dsreq_.ds_sensesent);
            break;
        case DSRT_NOSENSE:
            message(-2, "sense error");
            break;
        case DSRT_TIMEOUT:
            message(-2, "timeout");
            break;
        case DSRT_LONG:
            ...
            break;
        case DSRT_PROTO:
            message(-2, "protocol failure");
            break;
        case DSRT_EBSY:
            message(-2, "busy lost");
            break;
        case DSRT_REJECT:
            message(-2, "message reject");
            break;
        case DSRT_PARITY:
            message(-2, "parity error");
            break;
        case DSRT_MEMORY:
            message(-2, "memory error");
            break;
        case DSRT_CMDO:
            message(-2, "command error");
            break;
        case DSRT_STAI:
            message(-2, "status error");
            break;
        case DSRT_UNIMPL:
            message(-2, "not implemented");
            break;
        default:
            message(-2, "undefined ds_ret");
            break;
        }
        ...
        case 0x00:
            ...
            break;
        case 0x02:
            message(-2, "CHECK CONDITION");
            break;
        case 0x04:
            message(-2, "CONDITION MET");
            break;
        case 0x08:
            message(-2, "BUSY");
            break;
        case 0x10:
            ...
            break;
        case 0x14:
            message(-2, "INTERMEDIATE, CONDITION MET");
            break;
        case 0x18:
            message(-2, "RESERVATION CONFLICT");
            break;
        case 0x22:
            message(-2, "COMMAND TERMINATED");
            break;
        case 0x28:
            message(-2, "QUEUE FULL");
            break;
        case 0xff:
            break;
        default:
            message(-2, "undefined status");
        }
    }
}
...
int inq(int fd, void *sensebuf, int senselen, char *vend, char *prod, char *rev)
{
    ...
    struct dsreq dsreq = {/* flags */ DSRQ_READ,
                          /* time */ 1000,
                          /* private */ 0,
                          /* cmdbuf */ (caddr_t)cmd,
                          /* cmdlen */ sizeof(cmd),
    ...
    if (ioctl(fd, DS_ENTER, &dsreq) < 0 ||
        (dsreq.ds_ret != 0 && dsreq.ds_ret != DSRT_OK && dsreq.ds_ret != DSRT_SHORT)) {
...
ScsiIf::ScanData *ScsiIf::scan(int *len)
{
    ...
    *len = 0;
    ...
    while ((dp = readdir(dirp)) != NULL) {
        l = strlen(dp->d_name);
        if (*len < MAX_SCAN && l > 2 &&
            sscanf(dp->d_name, "sc%dd%dl%d%n", &bus, &targ, &lun, &count) == 3 && count == l) {
            ...
            if ((fd = open(s, O_RDWR, 0)) >= 0) {
                if (inq(fd, NULL, 0, scanData[*len].vendor, scanData[*len].product,
                        scanData[*len].revision) == 0)
                    (*len)++;
            }
            (void)close(fd);
        }
