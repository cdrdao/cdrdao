...
static int getSharedMemory(long nofBuffers, BufferHeader **header, long *nofSegments,
...
static int writer(const Toc *toc, CdrDriver *cdr, BufferHeader *header, long lba, int speed)
{
    ...
    if (cdr != NULL) {
        cdr->sendWriteCdProgressMsg(CdrDriver::WCD_LEADIN, totalTracks, 0, 0, 0, 100);
    ...
    do {
        ...
        if (header->readerFinished) {
            ...
            if (buffered == header->nofBuffers || buffered == header->nofBuffers - 1) {
                buffFill = 100;
            } else {
        ...
        if (len == 0) {
            ...
            if (cdr == NULL)
                ...
                log_message(1, "Wrote %ld blocks. Buffer fill min %d%%/max %d%%.", blkCount,
                            minFill, maxFill);
            ...
            if (cdr != NULL) {
                cdr->sendWriteCdProgressMsg(CdrDriver::WCD_LEADOUT, totalTracks, 0xaa, 1000, 1000,
                                            100);
        ...
        if (cdr != NULL) {
            ...
            if (cntMb > lastMb) {
                ...
                if (cdr->readBufferCapacity(&totalcap, &availcap)) {
                    writerFill = (int)((1.0 - ((double)availcap / (double)totalcap)) * 100.0);
                    log_message(1, "Wrote %ld of %ld MB (Buffers %3d%% %3d%%).\r", cnt >> 20,
                                total >> 20, buffFill, writerFill);
                } else {
                    log_message(1, "Wrote %ld of %ld MB (Buffer %3d%%).\r", cnt >> 20, total >> 20,
                                buffFill);
                }
                lastMb = cntMb;
            }
            ...
            cdr->sendWriteCdProgressMsg(CdrDriver::WCD_DATA, totalTracks, actTrackNr,
                                        buf.trackProgress, actProgress, buffFill, writerFill);
        } else {
            if (speed > 0) {
                log_message(1, "Read %ld of %ld MB (Buffer %3d%%).\r", cnt >> 20, total >> 20,
                            buffFill);
                mSleep(1000 / speed);
            } else {
...
static void *reader(void *args)
{
    ...
    do {
        ...
        do {
            rn = reader.readData(encodingMode, subChanEncodingMode, lba, buf.buffer, n);
        ...
        if (cdr != NULL && ((track->type() == TrackData::AUDIO && swap) ||
                            (encodingMode == 0 && cdr->bigEndianSamples() == 0 &&
                             (track->type() != TrackData::AUDIO)))) {
            // swap audio data
        ...
        while (header->buffersRead - header->buffersWritten == header->nofBuffers &&
...
int writeDiskAtOnce(const Toc *toc, CdrDriver *cdr, int nofBuffers, int swap, int testMode,
                    int speed)
{
    int err = 0;
    ...
    if (getSharedMemory(nofBuffers, &header, &nofShmSegments, &shmSegments) != 0) {
                           ...
                           ShmSegment **shmSegment)
{
    ...
    *nofSegments = 1;
    ...
    (*shmSegment)->buffer =
        new char[sizeof(BufferHeader) + nofBuffers * sizeof(Buffer) + nofBuffers * bufferSize];
    ...
    *header = (BufferHeader *)((*shmSegment)->buffer);
    ...
    (*header)->buffers = (Buffer *)((*shmSegment)->buffer + sizeof(BufferHeader));

    char *bufferBase = (*shmSegment)->buffer + sizeof(BufferHeader) + nofBuffers * sizeof(Buffer);
                           ...
                           ShmSegment **shmSegments)
{
    ...
    *shmSegments = NULL;
    ...
    if (shmctl(0, IPC_INFO, (struct shmid_ds *)&info) < 0) {
        ...
        log_message(-1, "Assuming %ld MB shared memory segment size.", maxSegmentSize >> 20);
    } else {
    ...
    log_message(4, "Shm max segment size: %ld (%ld MB)", maxSegmentSize, maxSegmentSize >> 20);
    ...
    *shmSegments = new ShmSegment[*nofSegments];
    ...
    for (i = 0; i < *nofSegments; i++) {
        ...
        if ((*shmSegments)[i].id < 0) {
            log_message(-2, "Cannot create shared memory segment: %s", strerror(errno));
            log_message(-2, "Try to reduce the buffer count (option --buffers).");
            return 1;
        }

        (*shmSegments)[i].buffer = (char *)shmat((*shmSegments)[i].id, 0, 0);
        if (((*shmSegments)[i].buffer) == NULL || ((*shmSegments)[i].buffer) == (char *)-1) {
            (*shmSegments)[i].buffer = NULL;
        ...
        if (*header == NULL) {
            bufferBase =
                (*shmSegments)[i].buffer + sizeof(BufferHeader) + nofBuffers * sizeof(Buffer);
            *header = (BufferHeader *)(*shmSegments)[i].buffer;
            (*header)->nofBuffers = nofBuffers;
            (*header)->buffers = (Buffer *)((*shmSegments)[i].buffer + sizeof(BufferHeader));
        } else {
