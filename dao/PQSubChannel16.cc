...
void PQSubChannel16::catalog(char n1, char n2, char n3, char n4, char n5, char n6, char n7, char n8,
                             char n9, char n10, char n11, char n12, char n13)
{
    assert(type_ == QMODE2);

    encodeCatalogNumber(data_ + 1, n1, n2, n3, n4, n5, n6, n7, n8, n9, n10, n11, n12, n13);
    data_[8] = 0;
}
...
const char *PQSubChannel16::catalog() const
{
    ...
    decodeCatalogNumber(data_ + 1, &buf[0], &buf[1], &buf[2], &buf[3], &buf[4], &buf[5], &buf[6],
...
void PQSubChannel16::isrc(char c1, char c2, char o1, char o2, char o3, char y1, char y2, char s1,
...
const char *PQSubChannel16::isrc() const
{
    ...
    decodeIsrcCode(data_ + 1, &buf[0], &buf[1], &buf[2], &buf[3], &buf[4], &buf[5], &buf[6],
...
void PQSubChannel16::print() const
{
    ...
    case QMODE1DATA:
    case QMODE5TOC:
        log_message(0, "Q: (%02x) %02x,%02x %02x:%02x:%02x %02x %02x:%02x:%02x ", data_[0],
                    data_[1], data_[2], data_[3], data_[4], data_[5], data_[6], data_[7], data_[8],
                    data_[9]);
        break;
    case QMODE2:
...
int PQSubChannel16::checkConsistency()
{
    switch (type_) {
    case QMODE1DATA:
        if (!isBcd(data_[3]) || !isBcd(data_[4]) || !isBcd(data_[5]) || !isBcd(data_[7]) ||
            !isBcd(data_[8]) || !isBcd(data_[9]))
            return 0;
        break;
