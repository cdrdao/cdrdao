...
#define PREFIX "/dev/"
    ...
    if (len == 0) {
        ...
        strcpy(impl_->name_, name);
               ...
               (strncmp(name, "sd", 2) == 0 || strncmp(name, "rsd", 3) == 0 ||
                strncmp(name, "cd", 2) == 0 || strncmp(name, "rcd", 3) == 0)) {
...
void ScsiIf::printError()
{
    ...
    if (impl_->error_ != NULL) {
        ...
        switch (impl_->screq_.retsts) {
        case SCCMD_OK:
            switch (impl_->screq_.status) {
            case 0x00:
                log_message(-2, "GOOD");
                break;
            case 0x02:
                log_message(-2, "CHECK CONDITION");
                break;
            case 0x04:
                log_message(-2, "CONDITION MET");
                break;
            case 0x08:
                log_message(-2, "BUSY");
                break;
            case 0x10:
                log_message(-2, "INTERMEDIATE");
                break;
            case 0x14:
                log_message(-2, "INTERMEDIATE, CONDITION MET");
                break;
            case 0x18:
                log_message(-2, "RESERVATION CONFLICT");
                break;
            case 0x22:
                log_message(-2, "COMMAND TERMINATED");
                break;
            case 0x28:
                log_message(-2, "QUEUE FULL");
                break;
            default:
                log_message(-2, "undefined status");
            }
            break;
        case SCCMD_TIMEOUT:
            log_message(-2, "timeout");
            break;
        case SCCMD_BUSY:
            log_message(-2, "busy");
            break;
        case SCCMD_SENSE:
            decodeSense(impl_->screq_.sense, impl_->screq_.senselen_used);
            break;
        case SCCMD_UNKNOWN:
            log_message(-2, "unknown error");
            break;
        default:
            log_message(-2, "undefined retsts");
            break;
        }
}
...
ScsiIf::ScanData *ScsiIf::scan(int *len, char *scsi_dev_path)
{
    ...
    *len = 0;
    ...
    while ((dp = readdir(dirp)) != NULL) {
        l = strlen(dp->d_name);
        if (*len < MAX_SCAN && l > 2 &&
            (((strncmp(dp->d_name, "rsd", 3) == 0 || strncmp(dp->d_name, "rcd", 3) == 0) &&
              isdigit(dp->d_name[l - 2]) && dp->d_name[l - 1] == c) ||
             ((strncmp(dp->d_name, "enrst", 5) == 0 || strncmp(dp->d_name, "ch", 2) == 0 ||
               strncmp(dp->d_name, "enss", 4) == 0 || strncmp(dp->d_name, "uk", 2) == 0) &&
            ...
            if ((fd = open(s, O_RDWR, 0)) >= 0) {
                if (ioctl(fd, SCIOCIDENTIFY, &saddr) >= 0) {
                    scanData[*len].dev = s;
                    if (inq(fd, scanData[*len].vendor, scanData[*len].product,
