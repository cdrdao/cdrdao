...
void PWSubChannel96::setChannelByte(Channel chan, int byteNr, unsigned char value)
{
...
int PWSubChannel96::checkCrc() const
{
    ...
    if (getChannelByte(Q_CHAN, 10) == (crc >> 8) && getChannelByte(Q_CHAN, 11) == (crc & 0xff))
        return 1;
    else
        return 0;
}
...
void PWSubChannel96::catalog(char n1, char n2, char n3, char n4, char n5, char n6, char n7, char n8,
                             char n9, char n10, char n11, char n12, char n13)
{
    assert(type_ == QMODE2);
    ...
    encodeCatalogNumber(buf, n1, n2, n3, n4, n5, n6, n7, n8, n9, n10, n11, n12, n13);
    buf[7] = 0;
...
const char *PWSubChannel96::catalog() const
{
    ...
    decodeCatalogNumber(in, &buf[0], &buf[1], &buf[2], &buf[3], &buf[4], &buf[5], &buf[6], &buf[7],
...
void PWSubChannel96::isrc(char c1, char c2, char o1, char o2, char o3, char y1, char y2, char s1,
...
const char *PWSubChannel96::isrc() const
{
    ...
    decodeIsrcCode(in, &buf[0], &buf[1], &buf[2], &buf[3], &buf[4], &buf[5], &buf[6], &buf[7],
...
void PWSubChannel96::print() const
{
    ...
    case QMODE5TOC:
        log_message(0, "Q: (%02x) %02x,%02x %02x:%02x:%02x %02x %02x:%02x:%02x ",
                    getChannelByte(Q_CHAN, 0), getChannelByte(Q_CHAN, 1), getChannelByte(Q_CHAN, 2),
                    getChannelByte(Q_CHAN, 3), getChannelByte(Q_CHAN, 4), getChannelByte(Q_CHAN, 5),
                    getChannelByte(Q_CHAN, 6), getChannelByte(Q_CHAN, 7), getChannelByte(Q_CHAN, 8),
                    getChannelByte(Q_CHAN, 9));
        break;
    case QMODE2:
        log_message(0, "Q: (%02x) MCN: %s      %02x ", getChannelByte(Q_CHAN, 0), catalog(),
                    getChannelByte(Q_CHAN, 9));
        break;
    case QMODE3:
        log_message(0, "Q: (%02x) ISRC: %s      %02x ", getChannelByte(Q_CHAN, 0), isrc(),
                    getChannelByte(Q_CHAN, 9));
        break;
    case QMODE_ILLEGAL:
    ...
    if (type_ != QMODE_ILLEGAL)
        log_message(0, "%04x %d", (getChannelByte(Q_CHAN, 10) << 8) | getChannelByte(Q_CHAN, 11),
                    checkCrc());
}
