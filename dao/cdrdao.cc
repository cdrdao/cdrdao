...
#include "trackdb/FormatConverter.h"
#include "Settings.h"
#include "port.h"
      ...
      no_utf8(false)
{
    ...
    fifoBuffers = 32;
#endif
    cddbServerList = {"gnudb.gnudb.org:80:/~cddb/cddb.cgi"};
...
Cdrdao::Cdrdao(std::optional<DaoCommandLine> options)
{
    if (options.has_value()) {
	errPrintParams.no_utf8 = options->no_utf8;
	filePrintParams.no_utf8 = options->no_utf8;
    } else {
	errPrintParams.no_utf8 = false;
	filePrintParams.no_utf8 = false;
    }
}

void Cdrdao::showDriveInfo(const DriveInfo *i)
{
    if (i == NULL) {
...
void Cdrdao::showTocInfo(const Toc *toc, const std::string &tocFile)
{
...
void Cdrdao::showTocSize(const Toc *toc, const std::string &tocFile)
{
    printf("%ld\n", toc->length().lba());
}

void Cdrdao::showToc(Toc *toc, DaoCommandLine &options)
{
    const Track *t;
    Msf start, end, index;
    int i;
    ...
    if (options.verbose > 2) {
	    ...
	    std::cout << "          ";
            item.print(std::cout, filePrintParams);
	    std::cout << "\n";
        }
    }
...
void Cdrdao::showData(const Toc *toc, bool swap)
{
...
void Cdrdao::ejectDisc(CdrDriver *cdr, DaoCommandLine &options)
{
    cdr->loadUnload(true);
}

void Cdrdao::showCDText(CdrDriver *cdr, DaoCommandLine &options)
{
    int nofTracks = 0;
...
void Cdrdao::showDiskInfo(DiskInfo *di)
{
    const char *s1, *s2;
 ...
 */
int Cdrdao::showMultiSessionInfo(DiskInfo *di)
{
...
void Cdrdao::printCddbQuery(Toc *toc)
{
    Cddb cddb(toc);
...
int Cdrdao::readCddb(const DaoCommandLine &opts, Toc *toc, bool showEntry)
{
    int err = 0;
    char *p;
    const char *sep = " ,";
...
int Cdrdao::checkToc(const Toc *toc, bool force)
{
    int ret = toc->check();
...
int Cdrdao::copyCd(DaoCommandLine &opts, CdrDriver *src, CdrDriver *dst)
{
    ...
    if (checkToc(toc, opts.force)) {
        ...
        toc->print(std::cout, errPrintParams);
        delete toc;
        return 1;
    }
...
int Cdrdao::copyCdOnTheFly(DaoCommandLine &opts, CdrDriver *src, CdrDriver *dst)
{
    Toc *toc = NULL;
    int pipeFds[2];
    pid_t pid = -1;
...
std::string Cdrdao::getDefaultDevice(DaoDeviceType req)
{
    int i, len;
    std::string buf;
...
CdrDriver *Cdrdao::setupDevice(DaoCommand cmd, const std::string &scsiDevice,
