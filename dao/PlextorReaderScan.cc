...
int PlextorReaderScan::analyzeTrack(TrackData::Mode mode, int trackNr, long startLba, long endLba,
...
int PlextorReaderScan::readSubChannels(TrackData::SubChannelMode sm, long lba, long len,
...
int PlextorReaderScan::readAudioRange(ReadDiskInfo *info, int fd, long start, long end,
                                      int startTrack, int endTrack, TrackInfo *trackInfo)
{
    if (!onTheFly_) {
        if ((options_ & OPT_PLEX_READ_ISRC) ||
            ...
            for (t = startTrack; t <= endTrack; t++) {
                ...
                sendReadCdProgressMsg(RCD_ANALYZING, info->tracks, t + 1, 0, totalProgress);
                ...
                totalProgress /= info->tracks;
                sendReadCdProgressMsg(RCD_ANALYZING, info->tracks, t + 1, 1000, totalProgress);
            }
    ...
    return CdrDriver::readAudioRangeParanoia(info, fd, start, end, startTrack, endTrack, trackInfo);
}
