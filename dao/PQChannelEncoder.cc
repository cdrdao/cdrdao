                                  ...
                                  const unsigned char *sheet, int len, const Msf &leadInStart)
{
    ...
    CueSheetEntry *run;
    for (run = actCueSheetEntry_, tocEnt = 0; (run->ctlAdr & 0x0f) != 1 || run->trackNr != 0xaa;
         run++) {
    ...
    for (ent = cueSheet_, i = 0; i < cueSheetLen_; ent++, i++) {
        ...
        case 1:
            if (ent->min > 99 || ent->sec > 59 || ent->frame > 74) {
                log_message(-3, "Illegal time field value at cue sheet entry: %d", i);
                return 1;
            }
            ...
            if (ent->trackNr == 0) {                       // indicates lead-in
                ...
                if (firstTrackNr_ == 0) {
                    ...
                    if (ent->trackNr != prevTrackNr && ent->trackNr != prevTrackNr + 1) {
                        log_message(-3, "Wrong track number sequence at cue sheet entry: %d", i);
                        return 1;
                    }
                    prevTrackNr = ent->trackNr;
                }
            ...
            if (ent->trackNr != 0) {
                ...
                if (lba <= prevLba) {
                    log_message(-3, "Time field does not increase at cue sheet entry: %d", i);
                    return 1;
                }
                prevLba = lba;
            }
        ...
        case 2:
            ...
            writeCatalog_ = 1;
            catalog_->catalog(ent->trackNr, ent->indexNr, ent->dataForm, ent->scms, ent->min,
                              ent->sec, ent->frame, cueSheet_[1].trackNr, cueSheet_[1].indexNr,
                              cueSheet_[1].dataForm, cueSheet_[1].scms, cueSheet_[1].min,
                              cueSheet_[1].sec);
            // skip next entry
            ent++;
            i++;
...
const SubChannel *PQChannelEncoder::encodeSubChannel(long lba)
{
    ...
    if (actCueSheetEntry_->trackNr == 0) {
        ...
        // catalog number
        if (writeCatalog_ && (deferredCatalog_ || (abslba_ % 90) == 0)) {
            if (newTransition) {
                deferredCatalog_ = 1;
            } else {
                deferredCatalog_ = 0;
        ...
        // ISRC code
        if (writeIsrc_ && (deferredIsrc_ || (abslba_ % 90) == 50 || (abslba_ % 90) == -40)) {
            if (newTransition) {
                deferredIsrc_ = 1;
            } else {
                deferredIsrc_ = 0;
        ...
        // P channel setup
        if (trlba_ <= 0 || (abslba_ >= nextTrackStartLba_ - 150 && abslba_ <= nextTrackStartLba_)) {
...
void PQChannelEncoder::nextTransition()
{
    ...
    if (nextEnt->trackNr != actCueSheetEntry_->trackNr) {
        ...
        if ((ent = nextCueSheetEntry(actCueSheetEntry_, 3)) != NULL &&
            ...
            ent1 = ent + 1;
            isrc_->isrc(ent->indexNr, ent->dataForm, ent->scms, ent->min, ent->sec, ent->frame,
                        ent1->indexNr, ent1->dataForm, ent1->scms, ent1->min, ent1->sec,
                        ent1->frame);
            writeIsrc_ = 1;
        } else {
            writeIsrc_ = 0;
    ...
    if (actCueSheetEntry_->trackNr != 0xaa) {
        ...
        if (actCueSheetEntry_->indexNr == 1) {
            ...
            if (actCueSheetEntry_->trackNr == lastTrackNr_)
                ...
                ent = nextCueSheetEntry(actCueSheetEntry_, actCueSheetEntry_->trackNr + 1, 1);
            assert(ent != NULL);
...
CueSheetEntry *PQChannelEncoder::nextCueSheetEntry(CueSheetEntry *act, int adr)
{
    if (act->trackNr == 0xaa) {
        return NULL;
    }
...
CueSheetEntry *PQChannelEncoder::nextCueSheetEntry(CueSheetEntry *act, int trackNr, int indexNr)
{
    if (act->trackNr == 0xaa) {
        return NULL;
    }
    ...
    while (act->ctlAdr != 0) {
        if ((act->ctlAdr & 0x0f) == 1 && act->trackNr == trackNr && act->indexNr == indexNr) {
            return act;
        }
        act++;
    }
