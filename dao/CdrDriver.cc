...
static DriverSelectTable BUILTIN_READ_DRIVER_TABLE[] = {
    ...
    {"generic-mmc", "PIONEER", "CD-ROM DR-U32",
     OPT_DRV_GET_TOC_GENERIC | OPT_MMC_USE_PQ | OPT_MMC_PQ_BCD, NULL},
    ...
    {"generic-mmc", "TEAC", "CD-524E", OPT_DRV_GET_TOC_GENERIC | OPT_MMC_USE_PQ | OPT_MMC_PQ_BCD,
     NULL},
...
static DriverTable DRIVERS[] = {{"cdd2600", &CDD2600::instance},
...
unsigned char CdrDriver::syncPattern[12] = {0,    0xff, 0xff, 0xff, 0xff, 0xff,
    ...
    while (fgets(buf, MAX_DRIVER_TABLE_LINE_LEN, fp) != NULL) {
        ...
        if ((p = strtok(l, sep)) != NULL) {
            if (strcmp(p, "R") == 0) {
                ...
                log_message(-1, "%s:%d: Expecting 'R' or 'W' as first token - line ignored.",
                            filename, lineNr);
            }
                ...
                if ((p = strtok(NULL, sep)) != NULL) {
                    ...
                    if ((p = strtok(NULL, sep)) != NULL) {
                        ...
                        if ((p = strtok(NULL, sep)) != NULL) {
                            ...
                            if (lastDriverName == NULL || strcmp(lastDriverName, driver) != 0) {
                                if ((driverName = checkDriverName(driver)) == NULL) {
                                    log_message(-1,
                                                "%s:%d: Driver '%s' not defined - line ignored.",
                            ...
                            while (!err && (p = strtok(NULL, sep)) != NULL) {
                                if ((opt = string2DriverOption(p)) == 0) {
                                    log_message(-1,
                                                "%s:%d: Driver option string '%s' not defined - "
                                                "line ignored.",
                                                filename, lineNr, p);
                                    err = 1;
                                }
                            ...
                            log_message(-1, "%s:%d: Missing driver name - line ignored.", filename,
                                        lineNr);
                        }
                    } else {
                        log_message(-1, "%s:%d: Missing model name - line ignored.", filename,
                                    lineNr);
                    }
                } else {
                    log_message(-1, "%s:%d: Missing vendor name - line ignored.", filename, lineNr);
                }
...
static void initDriverTable()
{
    ...
    if (readDriverTable(DRIVER_TABLE_FILE) != 0) {
        log_message(3, "Cannot read driver table from file \"%s\" - using built-in table.",
                    DRIVER_TABLE_FILE);
        createDriverTable();
    }
...
const char *CdrDriver::selectDriver(int readWrite, const char *vendor, const char *model,
                                    unsigned long *options)
{
    DriverSelectTable *run;
    ...
    while (run != NULL) {
        if (strcmp(run->vendor, vendor) == 0 && strstr(model, run->model) != NULL) {
...
int CdrDriver::cdrVendor(Msf &code, const char **vendorId, const char **mediumType)
{
...
int CdrDriver::sendCmd(const unsigned char *cmd, int cmdLen, const unsigned char *dataOut,
                       int dataOutLen, unsigned char *dataIn, int dataInLen, int showErrorMsg) const
{
    return scsiIf_->sendCmd(cmd, cmdLen, dataOut, dataOutLen, dataIn, dataInLen, showErrorMsg);
}
...
// return: 0: OK
//         1: scsi command failed
int CdrDriver::writeData(TrackData::Mode mode, TrackData::SubChannelMode sm, long &lba,
                         const char *buf, long len)
{
  ...
  log_message (0, "W: %ld: %ld, %ld, %ld", lba, blockLength, len, sum);
    ...
    while (len > 0) {
        ...
        if (sendCmd(cmd, 10, (unsigned char *)buf, writeLen * blockLength, NULL, 0) != 0) {
...
int CdrDriver::writeZeros(TrackData::Mode m, TrackData::SubChannelMode sm, long &lba, long encLba,
                          long count)
{
    assert(blocksPerWrite_ > 0);
  ...
  FILE *fp = fopen (fname, "w");
    ...
    while (count > 0) {
        ...
        if (writeData(encodingMode_ == 0 ? TrackData::AUDIO : m, sm, lba, zeroBuffer_, n) != 0) {
            return 1;
        }
...
int CdrDriver::getModePage(int pageCode, unsigned char *buf, long bufLen,
                           unsigned char *modePageHeader, unsigned char *blockDesc,
                           int showErrorMsg)
{
    unsigned char cmd[10];
...
// Return: 0: OK
//         1: SCSI command failed
int CdrDriver::setModePage(const unsigned char *modePage, size_t modePage_size,
                           const unsigned char *modePageHeader, const unsigned char *blockDesc,
                           int showErrorMsg)
{
...
{
    unsigned char cmd[6];
    long dataLen = bufLen + 4 /*mode parameter header */ + 100 /*spare for block descriptors */;
    unsigned char *data = new unsigned char[dataLen];
...
CdToc *CdrDriver::getToc(int sessionNr, int *cdTocLen)
{
    ...
    if (options_ & OPT_DRV_RAW_TOC_BCD) {
        if (isBcd == 0) {
            ...
            log_message(-1,
                        "Using TOC data retrieved with generic method (no multi session support).");
        ...
        if (isBcd == 0) {
            ...
            if (i < 0) {
                ...
                log_message(-1, "Your drive probably does not support raw toc reading.");
                log_message(
                    -1, "Using TOC data retrieved with generic method (no multi session support).");
                log_message(-1, "Use driver option 0x%lx to suppress this message.",
                            OPT_DRV_GET_TOC_GENERIC);
        ...
        if (isBcd == -1) {
            ...
            log_message(-1,
                        "Using TOC data retrieved with generic method (no multi session support).");
            log_message(-1, "Use driver option 0x%lx or 0x%lx to assume BCD or HEX data.",
    ...
    for (i = 0; i < rawTocLen; i++) {
        if (rawToc[i].sessionNr == sessionNr && (rawToc[i].adrCtl & 0xf0) == 0x10 && // QMODE1 entry
            rawToc[i].point < 0xa0) {

            if (isBcd) {
                trackNr = SubChannel::bcd2int(rawToc[i].point);
                trackStart =
                    Msf(SubChannel::bcd2int(rawToc[i].pmin), SubChannel::bcd2int(rawToc[i].psec),
                        SubChannel::bcd2int(rawToc[i].pframe))
                        .lba();
            } else {
                trackNr = rawToc[i].point;
                trackStart = Msf(rawToc[i].pmin, rawToc[i].psec, rawToc[i].pframe).lba();
            }
...
static char *buildDataFileName(int trackNr, CdToc *toc, int nofTracks, const char *basename,
...
Toc *CdrDriver::readDiskToc(int session, const char *dataFilename)
{
    ...
    for (i = 0; i < nofTracks; i++) {
        ...
        if ((cdToc[i].adrCtl & 0x04) != 0) {
            if ((trackMode = getTrackMode(i + 1, cdToc[i].start)) == TrackData::MODE0) {
                log_message(-1, "Cannot determine mode of data track %d - assuming MODE1.", i + 1);
            ...
            if (rawDataReading_) {
                ...
                if (trackMode == TrackData::MODE2_FORM1 || trackMode == TrackData::MODE2_FORM2) {
                    trackMode = TrackData::MODE2_FORM_MIX;
                }
            }
        } else {
    ...
    for (i = 0; i < nofTracks; i++) {
        ...
        if (!fastTocReading_) {
            ...
            if (trackInfos[i].mode == TrackData::AUDIO) {
                analyzeTrack(TrackData::AUDIO, i + 1, slba, elba, indexIncrements,
                             &indexIncrementCnt, i < nofTracks - 1 ? &pregap : 0, isrcCode,
                             &trackCtl);
                if (defaultPregap != 0)
        ...
        if ((trackCtl & 0x80) != 0) {
            ...
            if ((trackCtl & 0x01) != (cdToc[i].adrCtl & 0x01)) {
                log_message(
                    -1,
                    "Pre-emphasis flag of track differs from TOC - toc file contains TOC setting.");
                ctlCheckOk = 0;
            }
            if ((trackCtl & 0x08) != (cdToc[i].adrCtl & 0x08)) {
                log_message(-1, "2-/4-channel-audio  flag of track differs from TOC - toc file "
                                "contains TOC setting.");
                ctlCheckOk = 0;
            }
...
int CdrDriver::analyzeTrackSearch(TrackData::Mode, int trackNr, long startLba, long endLba,
                                  ...
                                  unsigned char *ctl)
{
    ...
    *indexCnt = 0;
    ...
    int dummy, track;
    if (getTrackIndex(startLba + 75, &track, &dummy, ctl) == 0 && track == trackNr) {
        *ctl |= 0x80;
    }
...
int CdrDriver::getTrackIndex(long lba, int *trackNr, int *indexNr, unsigned char *ctl)
{
    return 1;
}
...
long CdrDriver::findIndex(int track, int index, long trackStart, long trackEnd)
{
    int actTrack;
    int actIndex;
...
int CdrDriver::analyzeTrackScan(TrackData::Mode, int trackNr, long startLba, long endLba,
                                Msf *index, int *indexCnt, long *pregap, char *isrcCode,
                                unsigned char *ctl)
{
    ...
    *ctl = 0;
    ...
    while (length > 0) {
        ...
        if (readSubChannels(TrackData::SUBCHAN_NONE, startLba, n, &subChannels, NULL) != 0 ||
            subChannels == NULL) {
            return 1;
        }
        ...
        for (i = 0; i < n; i++) {
            ...
            if (chan->checkCrc() && chan->checkConsistency()) {
              ...
              if (chan->type () == SubChannel::QMODE1DATA)
                {
                  log_message (2,
                               "Q sub-channel data at %02d:%02d:%02d failed CRC check - ignored",
                               chan->min (), chan->sec (), chan->frame ());
                }
              else
                {
                  log_message (2,
                               "Q sub-channel data failed CRC check - ignored.");
                }
              chan->print ();
#endif
            }
...
int CdrDriver::checkToc(const Toc *toc)
{
    ...
    for (run = itr.first(), tracknr = 1; run != NULL; run = itr.next(), tracknr++) {
        if (run->subChannelType() != TrackData::SUBCHAN_NONE) {
            if (subChannelEncodingMode(run->subChannelType()) == -1) {
                log_message(-2, "Track %d: sub-channel writing mode is not supported by driver.",
                            tracknr);
                ret = 2;
            }
        }
...
long CdrDriver::blockSize(TrackData::Mode m, TrackData::SubChannelMode sm) const
{
    long bsize = 0;
    ...
    for (t = 0; t < tocLen; t++) {
        if (t == tocLen - 1) {
            log_message(1, "Leadout %s   %x      %s(%6ld)",
                        (toc[t].adrCtl & 0x04) != 0 ? "DATA " : "AUDIO", toc[t].adrCtl & 0x0f,
  ...
  for (i = 0; i < 15; i++)
    {
      ...
      for (j = 0; j < 4; j++)
        {
          log_message (0,
                       "%02x %02x %02x %02x: %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x  CRC: %02x %02x",
                       packs[j * 18 + 0], packs[j * 18 + 1],
                       packs[j * 18 + 2], packs[j * 18 + 3],
                       packs[j * 18 + 4], packs[j * 18 + 5],
                       packs[j * 18 + 6], packs[j * 18 + 7],
                       packs[j * 18 + 8], packs[j * 18 + 9],
                       packs[j * 18 + 10], packs[j * 18 + 11],
                       packs[j * 18 + 12], packs[j * 18 + 13],
                       packs[j * 18 + 14], packs[j * 18 + 15],
...
vector<CdTextItem *> processPacks(CdTextPack *packs, int nofPacks)
{
    ...
    while (nofPacks--) {
        ...
        log_message(4,
                    ...
                    "%02x  CRC: %02x %02x",
                    p.packType, p.trackNumber, p.sequenceNumber, p.blockCharacter, p.data[0],
                    p.data[1], p.data[2], p.data[3], p.data[4], p.data[5], p.data[6], p.data[7],
                    p.data[8], p.data[9], p.data[10], p.data[11], p.crc0, p.crc1);
        ...
        if (item->packType() == CdTextItem::PackType::SIZE_INFO) {
            ...
            if (item->dataLen() >= 1) {
                if (item->blockNr() >= 0 && item->blockNr() <= 8)
                    encodings[item->blockNr()] = Util::characterCodeToEncoding(data[0]);
            }
        }
    }
...
int CdrDriver::analyzeDataTrack(TrackData::Mode mode, int trackNr, long startLba, long endLba,
                                long *pregap)
{
    ...
    *pregap = 0;
    ...
    while (len > 0) {
        ...
        if ((actLen = readTrackData(mode, TrackData::SUBCHAN_NONE, lba, n, transferBuffer_)) < 0) {
        ...
        if (actLen != n) {
            ...
            *pregap = len - actLen;
            ...
            if (*pregap > 300) {
                log_message(-1, "The pre-gap of the following track appears to have length %s.",
                            Msf(*pregap).str());
...
Toc *CdrDriver::readDisk(int session, const char *dataFilename)
{
...
#ifdef __CYGWIN__
        if ((fp = open(dataFilename, O_WRONLY | O_CREAT | O_TRUNC | O_BINARY, 0666)) < 0)
#else
        if ((fp = open(dataFilename, O_WRONLY | O_CREAT | O_TRUNC, 0666)) < 0)
#endif
        {
            log_message(-2, "Cannot open \"%s\" for writing: %s", dataFilename, strerror(errno));
            delete[] cdToc;
            return NULL;
        }
    }
    ...
    while (trs < nofTracks) {
        if (trackInfos[trs].mode != TrackData::AUDIO) {
            ...
            if (session == 1 && trs == 0 && (options_ & OPT_DRV_NO_PREGAP_READ) == 0) {
                slba = 0;
            }
...
fail:
    ...
    if (!onTheFly_ && fp >= 0) {
        if (close(fp) != 0) {
            log_message(-2, "Writing to \"%s\" failed: %s", dataFilename, strerror(errno));
            delete toc;
            return NULL;
        }
    }
...
Toc *CdrDriver::buildToc(TrackInfo *trackInfos, long nofTrackInfos, int padFirstPregap)
{
    long i, j;
        ...
        if (trackMode == TrackData::AUDIO) {
            if (trackSubChanMode == TrackData::SUBCHAN_NONE) {
                if (newMode && (i > 0 || padFirstPregap)) {
                    ...
                    if (ati.pregap > 0) {
                        t.append(SubTrack(SubTrack::DATA, TrackData(Msf(ati.pregap).samples())));
                    }
                    t.append(SubTrack(SubTrack::DATA, TrackData(ati.filename, byteOffset, 0,
                                                                trackLength.samples())));
                } else {
                    Msf trackLength(nti.start - ati.start - nti.pregap + ati.pregap);
                    t.append(
                        SubTrack(SubTrack::DATA,
                                 TrackData(ati.filename, byteOffset,
                                           Msf(ati.start - modeStartLba - ati.pregap).samples(),
                                           trackLength.samples())));
                }
            } else {
                if (newMode && (i > 0 || padFirstPregap)) {
            ...
            if (ati.pregap != 0) {
                ...
                dataLen = ati.pregap * TrackData::dataBlockSize(trackMode, trackSubChanMode);
                t.append(SubTrack(SubTrack::DATA, TrackData(trackMode, trackSubChanMode, dataLen)));
            }

            dataLen = trackLength * TrackData::dataBlockSize(trackMode, trackSubChanMode);
            t.append(SubTrack(SubTrack::DATA, TrackData(trackMode, trackSubChanMode, ati.filename,
                                                        byteOffset, dataLen)));

            if (ati.fill > 0) {
                dataLen = ati.fill * TrackData::dataBlockSize(trackMode, trackSubChanMode);
                t.append(SubTrack(SubTrack::DATA, TrackData(trackMode, trackSubChanMode, dataLen)));
            }
...
// Return: 0: OK
//         1: error occured
int CdrDriver::readDataTrack(ReadDiskInfo *info, int fd, long start, long end, TrackInfo *trackInfo)
{
    long len = end - start;
    long totalLen = len;
    long lba;
        ...
        if (foundLECError) {
            ...
            if (lba > lastLba + 75) {
                ...
                if (remote_) {
                    ...
                    sendReadCdProgressMsg(RCD_EXTRACTING, info->tracks, trackInfo->trackNr,
...
void CdrDriver::sendReadCdProgressMsg(ReadCdProgressType type, int totalTracks, int track,
...
int CdrDriver::sendWriteCdProgressMsg(WriteCdProgressType type, int totalTracks, int track,
                                      int trackProgress, int totalProgress, int bufferFillRate,
                                      int writeFill)
{
    if (remote_) {
        int fd = remoteFd_;
...
long CdrDriver::audioRead(TrackData::SubChannelMode sm, int byteOrder, Sample *buffer,
                          long startLba, long len)
{
    SubChannel **chans;
    int i;
    int swap;
    ...
    if (remote_ && startLba > audioReadLastLba_) {
        ...
        sendReadCdProgressMsg(RCD_EXTRACTING, audioReadInfo_->tracks, audioReadActTrack_ + 1,
                              progress, totalProgress);
        ...
        if (chan->checkCrc() && chan->checkConsistency()) {
            if (chan->type() == SubChannel::QMODE1DATA) {
                ...
                if (t >= audioReadStartTrack_ && t <= audioReadEndTrack_ &&
                    ...
                    if (t == audioReadActTrack_ && chan->indexNr() == audioReadActIndex_ + 1) {

                        if (chan->indexNr() > 1) {
                            log_message(2, "Found index %d at: %s", chan->indexNr(), time.str());

                            if (audioReadTrackInfo_[t].indexCnt < 98) {
                                audioReadTrackInfo_[t].index[audioReadTrackInfo_[t].indexCnt] =
                                    time.lba();
                ...
                if (audioReadTrackInfo_[audioReadActTrack_].isrcCode[0] == 0) {
                    log_message(2, "Found ISRC code.");
                    strcpy(audioReadTrackInfo_[audioReadActTrack_].isrcCode, chan->isrc());
                }
            }
        } else {
            audioReadCrcCount_++;
...
int CdrDriver::readAudioRangeStream(ReadDiskInfo *info, int fd, long start, long end,
                                    int startTrack, int endTrack, TrackInfo *trackInfo)
{
    ...
    while (len > 0) {
        ...
        CdrDriver::audioRead(subChanReadMode_, 1 /*big endian byte order */, (Sample *)buf, lba, n);
...
int CdrDriver::readAudioRangeParanoia(ReadDiskInfo *info, int fd, long start, long end,
                                      int startTrack, int endTrack, TrackInfo *trackInfo)
{
    long startLba = start;
    long endLba = end - 1;
...
long cdda_read(cdrom_drive *d, void *buffer, long beginsector, long sectors)
{
    ...
    return cdr->audioRead(TrackData::SUBCHAN_NONE, cdr->hostByteOrder(), (Sample *)buffer,
                          beginsector, sectors);
}
...
{
    static char line[80];
    snprintf(line, sizeof(line), "type %02x track %2d seq %3d bc %02x : ", pack.packType,
             pack.trackNumber, pack.sequenceNumber, pack.blockCharacter);
    os << line;
    for (int i = 0; i < 12; i++) {
