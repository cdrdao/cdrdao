...
int YamahaCDR10x::selectSpeed()
{
    ...
    if (getModePage6(0x31 /*drive configuration mode page*/, mp, 4, NULL, NULL, 1) != 0) {
    ...
    if (speed_ == 0 || speed_ == 4) {
        ...
        log_message(-2, "Invalid write speed argument %d. 0, 1, 2 or 4 expected.", speed_);
        return 1;
    }
...
int YamahaCDR10x::setWriteParameters()
{
    ...
    if (getModePage6(0x32 /*write format mode page*/, mp, 8, NULL, NULL, 1) != 0) {
...
void YamahaCDR10x::cueSheetDataType(TrackData::Mode mode, unsigned char *dataType,
...
int YamahaCDR10x::startDao()
{
    ...
    if (setWriteParameters() != 0 || sendCueSheet() != 0) {
        return 1;
    }
...
int YamahaCDR10x::finishDao()
{
    ...
    while (sendCmd(cmd, 6, NULL, 0, NULL, 0, 0) == 2 && (sense = scsiIf_->getSense(senseLen)) &&
           senseLen && sense[2] == 0x0b && sense[0xc] == 0x50) {
        sleep(1);
    }
...
int YamahaCDR10x::analyzeTrack(TrackData::Mode mode, int trackNr, long startLba, long endLba,
...
int YamahaCDR10x::readSubChannels(TrackData::SubChannelMode, long lba, long len,
...
// return: 0: OK
//         1: scsi command failed
int YamahaCDR10x::writeData(TrackData::Mode mode, TrackData::SubChannelMode sm, long &lba,
                            const char *buf, long len)
{
  ...
  log_message(0, "W: %ld: %ld, %ld, %ld", lba, blockLength, len, sum);
    ...
    while (len > 0) {
        ...
        do {
            retry = 0;
            retval = sendCmd(cmd, 10, (unsigned char *)buf, writeLen * blockLength, NULL, 0, 0);
            if (retval == 2) {
                const unsigned char *sense;
                int senseLen;
...
long YamahaCDR10x::readTrackData(TrackData::Mode mode, TrackData::SubChannelMode, long lba,
...
int YamahaCDR10x::readAudioRange(ReadDiskInfo *rinfo, int fd, long start, long end, int startTrack,
