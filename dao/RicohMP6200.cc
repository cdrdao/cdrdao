...
int RicohMP6200::initDao(const Toc *toc)
{
    ...
    if (readSessionInfo(&leadInLen_, &leadOutLen_, 1) != 0 || setWriteParameters() != 0)
        return 1;
...
int RicohMP6200::startDao()
{
    ...
    // write lead-in
    if (writeZeros(toc_->leadInMode(), TrackData::SUBCHAN_NONE, lba, 0, leadInLen_) != 0) {
        flushCache();
        return 1;
    }
...
int RicohMP6200::finishDao()
{
    ...
    // write lead-out
    if (writeZeros(toc_->leadOutMode(), TrackData::SUBCHAN_NONE, lba, lba + 150, leadOutLen_) !=
        0) {
        flushCache();
        return 1;
    }
...
// return: 0: OK
//         1: scsi command failed
int RicohMP6200::writeData(TrackData::Mode mode, TrackData::SubChannelMode sm, long &lba,
