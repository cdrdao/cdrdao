...
class PlextorReader : public CdrDriver
{
  public:
    ...
    inline int ReadErrorsSlowDown()
    {
        return slow_down_on_read_errors;
    }
    inline int VibrationsSlowDown()
    {
        return slow_down_on_vibrations;
    }
    inline int WaitMaxSpeed()
    {
        return transfer_data_before_max_speed;
    }
  ...
  protected:
    ...
    */
    ...
    long readTrackData(TrackData::Mode mode, TrackData::SubChannelMode, long lba, long len,
                       unsigned char *buf);
  ...
  private:
    ...
    int readSubChannelData(int *trackNr, int *indexNr, long *, unsigned char *ctl);

    int readAudioRangePlextor(ReadDiskInfo *, int fd, long start, long end, int startTrack,
